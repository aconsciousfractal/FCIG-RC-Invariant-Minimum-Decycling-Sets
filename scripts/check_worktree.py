...
REQUIRED_FILES = {
    ...
    "paper/sections/07_direct_spectral_construction.tex",
    "paper/sections/08_qary_parity_theorem.tex",
    "paper/sections/09_exact_validation_and_reproducibility.tex",
    "paper/sections/10_boundaries_and_outlook.tex",
...
PRIVATE_PATTERNS = {
    # Require either a genuine directory component or a root-level filename;
    # this rejects native paths without mistaking TeX such as ``s:\A`` or
    # ``w:\tail(w)`` for one.
...
def main() -> None:
    ...
    if "MIT License" not in licence or "Permission is hereby granted" not in licence:
        failures.append("MIT licence notice missing or incomplete")
